...
type MigrationDb = Pick<Pool, "execute">;

const CREATE_WIKI_MEDIA = `CREATE TABLE IF NOT EXISTS wiki_media (
...
const CREATE_WIKI_PAGE_READERS = `CREATE TABLE IF NOT EXISTS wiki_page_readers (
...
export async function applyWikiMediaReaderMigration(db: MigrationDb = getPool()) {
  await db.execute(CREATE_WIKI_MEDIA);
  await db.execute(CREATE_WIKI_PAGE_READERS);
}
